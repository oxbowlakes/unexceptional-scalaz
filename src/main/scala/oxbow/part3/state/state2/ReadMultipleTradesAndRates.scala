package oxbow.part3.state.state2
...
object ReadMultipleTradesAndRates extends App {
  sealed trait E
  ...
  val pathsToRates = List("/mnt/live/rates/deutsche/2015/09/trades-20150919.csv")



  import collection.JavaConverters._
  def readAllLines(pathToRates: String): E \/ Stream[String] = \/.fromTryCatchNonFatal(Files.readAllLines(Paths.get(getClass.getResource(pathToRates).toURI))).bimap(IOException, _.asScala.toStream)
  ...
  //I now have to fold the computation through a List, say
  type M[A] = EitherT[List, E, A]
  val rates =
    for {
      path <- EitherT.right(pathsToRates)
      lines <- EitherT(List(readAllLines(path)))
      f <- EitherT(List(toCsv(lines)))
      y <- EitherT(List(f.parse))
      (rs, ts) = y
      v <- EitherT(List(rs.verifiedUSD))
    } yield (v, ts)

  println(rates.sequenceU)
}
